    ...
    export default {
        ...
        methods: {
            ...
            addTodoTag: function (todoId, tagId) {
                ...
                axios.defaults.headers['content-tyoe'] = 'application/json';
                axios.post('/goals\${this.goalId}/tags/${tagId}').then((response) => {
                    this.todos.length = 0;
                    for (let i = 0; i < response.data.length; i++) {
                        this.todos.push(response.data[i])
                    }
            ...
            editTodoContent: function (todo) {
                axios.defaults.headers['X-CSRF-TOKEN'] = $('meta[name=csrf-token]').attr('content');
                axios.defaults.headers['contetent-type'] = 'application'/json;
